    ...
    bot.sendMessage(chatId, `🎛 **پنل مدیریت پیشرفته آرنا**\n\nبخش مورد نظر را انتخاب کنید:`, {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: "📊 آمار و گزارشات جامع", callback_data: "adm_stats" }]
            ]
        },
        parse_mode: "Markdown"
    });
...
bot.on("callback_query", async (query) => {
    ...
    if (data === "sub_list" && isAdmin(query.from)) {
        db.all(`SELECT * FROM products`, (err, rows) => {
            ...
            rows.forEach((s, idx) => {
                textMsg += `${idx + 1}. **${s.name}** - ${s.price.toLocaleString()} تومان\n` +
                           `   - حجم: ${s.volume} | کانفیگ: \`${s.config}\`\n-------------------\n`;
            });
...
bot.on("message", (msg) => {
    ...
    if (isAdmin(msg.from)) {
        ...
        if (currentState === "sub_add_config") {
            ...
            userState[userId].step = null;
            bot.sendMessage(chatId, `✅ محصول جدید با موفقیت ثبت شد.`);
            return;
        }
    }
