...
bot.onText(/🖥️ پنل مدیریت|\/panel/, async (msg) => {
    loadDatabase();
    const chatId = msg.chat.id;
    if (!isAdmin(msg)) {
...
function sendAdminPanel(chatId) {
    ...
    const adminKeyboard = {
        reply_markup: {
            inline_keyboard: [
                ...
                [
                    { text: '👥 مدیریت کاربران', callback_data: 'manage_wallets' },
                    { text: '📦 مدیریت اشتراک‌ها', callback_data: 'manage_user_subs' }
                ],
                [
                ...
                [
                    { text: '🎟️ کدهای تخفیف', callback_data: 'admin_discount_menu' },
                    { text: `📊 آمار ربات (${uniqueUsersCount})`, callback_data: 'admin_stats' }
                ],
                [
    ...
    bot.sendMessage(chatId, '⚙️ **پنل مدیریت ربات**\nلطفاً گزینه مورد نظر را انتخاب کنید: 👇', {
        parse_mode: 'Markdown',
        ...adminKeyboard
    }).catch(() => {});
}
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data.startsWith('approve_deposit_') || data.startsWith('reject_deposit_')) {
        ...
        if (action === 'approve') {
            ...
            if (depositInfo) {
                ...
                bot.sendMessage(CHANNEL_LOG_ID, channelDepositMsg, { parse_mode: 'Markdown' }).catch(() => {});
            ...
            const channelRejectMsg = `❌ **رد رسید شارژ کیف پول:**\n` +
                                     `👤 کاربر: @${cleanUName} (\`${targetUserId}\`)\n` +
                                     `🕒 زمان: ${getPersianDateTime()}`;
            bot.sendMessage(CHANNEL_LOG_ID, channelRejectMsg, { parse_mode: 'Markdown' }).catch(() => {});
    ...
    if (data.startsWith('approve_card_') || data.startsWith('reject_card_')) {
        ...
        if (action === 'approve') {
            ...
            if (plan && plan.links.length > 0) {
                ...
                const purchaseMessage = `🛒 **خرید کارت به کارت جدید و تایید شد (تحویل کانفیگ):**\n` +
                                        `👤 **نام کاربری:** @${cleanUsername} (\`${targetUserId}\`)\n` +
                                        `📦 **پلن:** ${plan.name}\n` +
                                        `⏰ **زمان:** ${currentDateStr}\n` +
                                        `🌐 **حجم:** ${parsedData.total !== 'نامشخص' ? parsedData.total : plan.volume}\n\n` +
                                        `🔗 **لینک اشتراک:**\n\`${assignedLink}\``;
                
                let channelFullLog = purchaseMessage;
                ...
                await bot.sendMessage(CHANNEL_LOG_ID, channelFullLog, { parse_mode: 'Markdown', ...channelKeyboard }).catch(() => {});
            ...
            const channelRejectCardMsg = `❌ **رد رسید خرید کارت به کارت:**\n` +
                                         `👤 کاربر: @${cleanUsername} (\`${targetUserId}\`)\n` +
                                         `🕒 زمان: ${getPersianDateTime()}`;
            bot.sendMessage(CHANNEL_LOG_ID, channelRejectCardMsg, { parse_mode: 'Markdown' }).catch(() => {});
    ...
    if (data === 'restart_bot') {
        ...
        if (isAdmin(callbackQuery)) {
            bot.sendMessage(chatId, '👑 پنل مدیریت ربات:', {
                reply_markup: {
                    keyboard: [[{ text: '🖥️ پنل مدیریت' }], [{ text: '🚪 بستن ربات و خروج' }]],
                    resize_keyboard: true,
                    is_persistent: true
                }
            });
    ...
    if (data.startsWith('pay_wallet_')) {
        ...
        let channelLogMsg = `🛒 **خرید موفق از کیف پول (تحویل کانفیگ):**\n` +
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('/start') || text === '🖥️ پنل مدیریت' || text === '/panel') {
        return;
    }
    ...
    if (currentState.step === 'get_wallet_deposit_receipt' && (msg.photo || msg.document)) {
        ...
        const adminCaption = `💳 **رسید شارژ کیف پول جدید (ارسال شده توسط کاربر)**\n\n` +
                             `👤 نام: ${userInfo.name}\n` +
                             `🔗 یوزرنیم: ${userInfo.username}\n` +
                             `🆔 شناسه: \`${userId}\`\n` +
    ...
    if (currentState.step === 'get_card_purchase_receipt' && (msg.photo || msg.document)) {
        ...
        const adminCaption = `💳 **رسید خرید کارت به کارت جدید (ارسال شده توسط کاربر)**\n\n` +
                             `👤 نام: ${userInfo.name}\n` +
                             `🔗 یوزرنیم: ${userInfo.username}\n` +
                             `🆔 شناسه: \`${userId}\`\n` +
                             `📦 پلن درخواستی: \`${plan.name}\`\n` +
                             `💵 مبلغ: **${priceNumber.toLocaleString()} تومان**\n\n` +
                             `لطفاً رسید را بررسی کرده و در صورت صحت، تایید نمایید:`;
        ...
        if (msg.photo) {
            bot.sendPhoto(ADMIN_CHAT_ID, fileId, { caption: adminCaption, parse_mode: 'Markdown', ...approvalKeyboard }).catch(() => {});
            bot.sendPhoto(CHANNEL_LOG_ID, fileId, { caption: `📥 **رسید خرید کارت به کارت (در انتظار تایید جهت ارسال):**\n👤 کاربر: @${(userInfo.username || 'ندارد').replace('@', '')} (\`${userId}\`)\n📦 پلن: ${plan.name}\n💵 مبلغ: ${priceNumber.toLocaleString()} تومان`, parse_mode: 'Markdown' }).catch(() => {});
        } else {
            bot.sendDocument(ADMIN_CHAT_ID, fileId, { caption: adminCaption, parse_mode: 'Markdown', ...approvalKeyboard }).catch(() => {});
            bot.sendDocument(CHANNEL_LOG_ID, fileId, { caption: `📥 **رسید خرید کارت به کارت (در انتظار تایید جهت ارسال):**\n👤 کاربر: @${(userInfo.username || 'ندارد').replace('@', '')} (\`${userId}\`)\n📦 پلن: ${plan.name}\n💵 مبلغ: ${priceNumber.toLocaleString()} تومان`, parse_mode: 'Markdown' }).catch(() => {});
        }
    ...
    if (currentState.step === 'get_broadcast_content') {
        ...
        for (const uId of allUsersList) {
            try {
                if (msg.photo) {
                    ...
                    await bot.sendPhoto(uId, photoId, { caption: msg.caption || '', parse_mode: 'Markdown' });
        ...
        bot.sendMessage(chatId, `✅ **پیام همگانی با موفقیت ارسال شد.**\n\n📤 ارسال موفق: ${successCount}\n❌ ارسال ناموفق (بلاک شده یا غیرفعال): ${failCount}`, { parse_mode: 'Markdown' }).catch(() => {});
        return;
    }
    ...
    if (currentState.step === 'get_new_plan_name') {
        if (!isAdmin(msg)) return;
        const planName = text;
        db.userStates[chatId] = { step: 'get_new_plan_volume', tempPlan: { name: planName } };
        saveDatabase();
        bot.sendMessage(chatId, '🌐 لطفاً **حجم پلن** را وارد کنید (مثلاً `50 گیگابایت` یا `نامحدود`):').catch(() => {});
        return;
    }

    if (currentState.step === 'get_new_plan_volume') {
        if (!isAdmin(msg)) return;
        currentState.tempPlan.volume = text;
    ...
    if (currentState.step === 'get_new_plan_duration') {
        if (!isAdmin(msg)) return;
        currentState.tempPlan.duration = text;
    ...
    if (currentState.step === 'get_new_plan_price') {
        if (!isAdmin(msg)) return;
        currentState.tempPlan.price = text;
        currentState.step = 'get_new_plan_links';
        currentState.tempPlan.links = [];
        saveDatabase();
        bot.sendMessage(chatId, '🔗 لطفاً **لینک‌های سابسکریپشن یا کانفیگ‌ها** را (هر کدام در یک خط یا یکجا) ارسال کنید:\n*(پس از ارسال تمام لینک‌ها، عبارت `تایید` را بفرستید)*').catch(() => {});
        return;
    }
    ...
    if (currentState.step === 'get_new_plan_links') {
        ...
        if (text === 'تایید') {
            const newPlan = {
                ...
                price: currentState.tempPlan.price,
                links: currentState.tempPlan.links
            };
            ...
            currentState.tempPlan.links.push(...lines);
            saveDatabase();
            bot.sendMessage(chatId, `📥 ${lines.length} لینک اضافه شد.\nمجموع لینک‌های این پلن: ${currentState.tempPlan.links.length} عدد.\n\nاگر لینک دیگری دارید بفرستید، در غیر این صورت عبارت ` + '`تایید`' + ` را ارسال کنید.`, { parse_mode: 'Markdown' }).catch(() => {});
            return;
        }
    }
    ...
    if (currentState.step === 'edit_plan_field_value') {
        ...
        if (plan) {
            ...
            delete db.userStates[chatId];
            saveDatabase();
            bot.sendMessage(chatId, `✅ مشخصات پلن با موفقیت به روز شد.`).catch(() => {});
        } else {
            bot.sendMessage(chatId, '❌ پلن یافت نشد.');
        }
        return;
    ...
    if (currentState.step === 'get_new_discount_code') {
        ...
        currentState.tempCode = code;
        currentState.step = 'get_new_discount_percent';
        saveDatabase();
        bot.sendMessage(chatId, '🎟 لطفاً **درصد تخفیف** را به عدد وارد کنید (مثلاً `20`):').catch(() => {});
        return;
    }
    ...
    if (currentState.step === 'get_new_discount_percent') {
        ...
        bot.sendMessage(chatId, `✅ کد تخفیف \`${code}\` با تخفیف **${percent}%** با موفقیت ثبت شد.`, { parse_mode: 'Markdown' }).catch(() => {});
        sendAdminPanel(chatId);
        return;
    }
