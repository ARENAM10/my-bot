...
bot.on("message", (msg) => {
    ...
    if (isAdmin(msg.from)) {
        ...
        if (currentState === "add_prod_duration") {
            ...
            userState[userId].step = null;
            bot.sendMessage(chatId, `✅ محصول "${p.name}" با موفقیت ساخته شد!`);
            openAdminPanel(chatId);
            return;
        }
        ...
        if (currentState === "add_config_to_pool") {
            ...
            userState[userId].step = null;
            bot.sendMessage(chatId, `✅ تعداد ${addedCount} کانفیگ جدید به انبار اضافه شد.`);
            openAdminPanel(chatId);
            return;
        }
    }
    ...
    if (currentState === "waiting_for_receipt") {
        ...
        db.get(`SELECT * FROM orders WHERE orderId = ?`, [orderId], (err, order) => {
            if (order && (photo || text)) {
                ...
                bot.sendMessage(chatId, "✅ فیش شما ثبت شد. پس از تأیید مدیریت، کانفیگ شما به صورت خودکار ارسال خواهد شد.", mainKeyboard);
                userState[userId].step = null;
            }
        });
        return;
...
function openAdminPanel(chatId) {
    db.all(`SELECT p.id, p.name, (SELECT COUNT(*) FROM config_pool cp WHERE cp.productId = p.id AND cp.status = 0) as stock FROM products p`, (err, rows) => {
        let textMsg = "🎛 **پنل مدیریت انبار و محصولات**\n\nلیست محصولات و موجودی انبار:\n\n";
        let inlineKeys = [];

        if (rows && rows.length > 0) {
            rows.forEach((r, idx) => {
                textMsg += `${idx + 1}. **${r.name}** 📦 موجودی: \`${r.stock} عدد\`\n`;
...
bot.on("callback_query", async (query) => {
    ...
    if (data.startsWith("admin_add_cfg_") && isAdmin(query.from)) {
        ...
        bot.sendMessage(chatId, "🔗 لینک‌های کانفیگ را بفرستید (هر کانفیگ در یک خط جدید):");
        return;
    }
    ...
    if (data.startsWith("approve_order_") && isAdmin(query.from)) {
        ...
        db.get(`SELECT * FROM orders WHERE orderId = ?`, [orderId], (err, order) => {
            ...
            deliverConfigToUser(order.userId, order.userId, order.productId, (success) => {
                if (success) {
                    bot.sendMessage(chatId, `✅ سفارش ${orderId} تأیید شد و کانفیگ به صورت خودکار برای کاربر ارسال گردید.`);
                } else {
                    bot.sendMessage(chatId, `⚠️ سفارش تأیید شد اما انبار کانفیگ این محصول خالی بود!`);
                }
            });
        });
        return;
    ...
    if (data === "wallet_history") {
        db.all(`SELECT * FROM transactions WHERE userId = ? ORDER BY id DESC LIMIT 5`, [userId], (err, rows) => {
            if (!rows || rows.length === 0) {
                bot.sendMessage(chatId, "📜 هیچ تراکنشی ثبت نشده است.");
                return;
            }
    ...
    if (data === "wallet_charge") {
        bot.sendMessage(chatId, `💳 برای شارژ کیف پول، لطفاً به پشتیبانی (@${ADMIN_USERNAME}) پیام دهید.`);
        return;
    }
...
console.log("ربات با سیستم کامل انبار و تحویل خودکار روی index.js راه‌اندازی شد...");
